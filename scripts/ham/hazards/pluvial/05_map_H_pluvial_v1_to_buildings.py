...
GRID_PATH = Path("processed/HAM/hazards/pluvial/H_pluvial_v1_grid.nc")
BUILDINGS_PATH = Path("processed/HAM/derived/buildings_ham.gpkg")
OUT_PATH = Path("processed/HAM/hazards/pluvial/H_pluvial_v1_buildings.parquet")
...
def fill_grid_nans_kdtree(H: xr.DataArray) -> xr.DataArray:
    ...
    Uses SciPy cKDTree if available, otherwise brute force.

    This is a pragmatic fix for isolated NaNs (e.g. land/sea mask artifacts).
    """
...
def main() -> None:
    ...
    try:
        ...
        if "bldg_id" not in gdf.columns:
            raise SystemExit("buildings_ham.gpkg missing 'bldg_id' column.")
        if gdf.crs is None:
            raise SystemExit("buildings_ham.gpkg has no CRS. Expected projected Hamburg CRS.")

        print("Loaded buildings:", len(gdf))
        print("Buildings CRS:", gdf.crs)

        # --- Representative points / centroids to WGS84 (ERA5 grid is lat/lon) ---
