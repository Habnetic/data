...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "RTM Phase 1: Validate ERA5-Land hourly tp monthly files for a given year, "
...
def main() -> None:
    ...
    files = sorted(raw_dir.glob(f"era5_land_tp_hourly_{year}-*.nc"))
    ...
    try:
        ...
        # ERA5-Land tp is typically in meters; Phase 1 hazard spec locks mm.
        tp_mm = ds["tp"] * 1000.0
        # Preserve metadata where possible, but override unit.
        try:
        ...
        out_path = out_dir / f"era5_land_tp_amax_{year}_RTM.nc"
