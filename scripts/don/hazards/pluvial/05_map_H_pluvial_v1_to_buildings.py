...
GRID_PATH = Path("processed/DON/hazards/pluvial/H_pluvial_v1_grid.nc")
BUILDINGS_PATH = Path("processed/DON/derived/buildings_don.gpkg")
OUT_PATH = Path("processed/DON/hazards/pluvial/H_pluvial_v1_buildings.parquet")
...
def main() -> None:
    ...
    try:
        ...
        if "bldg_id" not in gdf.columns:
            raise SystemExit("buildings_don.gpkg missing 'bldg_id' column.")
        if gdf.crs is None:
            raise SystemExit("buildings_don.gpkg has no CRS. Expected EPSG:25830.")
