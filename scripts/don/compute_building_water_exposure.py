...
def read_gpkg(path: Path) -> gpd.GeoDataFrame:
    ...
    if gdf.crs is None:
        raise ValueError(f"{path} has no CRS. Expected EPSG:25830.")
    return gdf


def ensure_crs(gdf: gpd.GeoDataFrame, epsg: int = 25830) -> gpd.GeoDataFrame:
...
def main() -> None:
    ...
    # Keep line geometries only after clip artifacts
    water = water[water.geometry.geom_type.isin(["LineString", "MultiLineString"])].copy()
