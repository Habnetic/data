...
class Paths:
    buildings_norm: Path = Path("processed/DON/normalized/buildings.gpkg")
    buildings_layer: str = "buildings"

    boundary_norm: Path = Path("processed/DON/normalized/boundary_don.gpkg")
    boundary_layer: str | None = None  # None = first layer

    out_gpkg: Path = Path("processed/DON/derived/buildings_don.gpkg")
    out_layer: str = "buildings_don"
...
def main() -> int:
    ...
    if "bldg_id" not in buildings.columns:
        raise ValueError(
            ...
            "Re-run scripts/don/normalize_buildings.py (it must write bldg_id)."
        )

    print("Reading DON boundary (normalized)...")
    ...
    print("Clipping buildings to DON boundary...")
